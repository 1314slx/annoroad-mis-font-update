...
export default class Credit extends Component {
      ...
      <div>
        ...
        {value.status === 3 && (
          <Button
            ...
            size="small"
            className={styles.dashedBtn}
            onClick={this.handleUpData.bind(this, value, 2)}
          >
            更新附件
          </Button>
        )}
  ...
  handleLook = value => {
    ...
    //点击"查看"按钮  设置各种状态
    this.setLookPageStatus(value, 1, 1, 1, 1);
  ...
  handleCredit = value => {
    ...
    //进入 create页面 不调取 授信申请详情查询接口,切点击 返回 提交 保存 返回 授信管理列表页面
    this.setCreatePageStatus(value, 1, 1);
  ...
  handleUpData = value => {
    this.packageLocalStorage(value);
    this.props.history.push("/creditManage/upData");
  };
  ...
  render() {
    ...
    const { datas, page_no, total } = creditData;
