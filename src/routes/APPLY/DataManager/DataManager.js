...
import FileTree from "./CopyFile/FileTree";
    ...
    new Promise((resolve) => {
      const detial = (data) => {
        ...
        request("/annoroad-cloud-mis-server/data/signature-url",
          { body: { "fileUrl": record && record.path?(record.path+ "/" +record.file_name):(this.props.dataManager.currentPath + "/" + name) } })
          .then((data) => {
            if (data.code != "000000") {
              notification.error({
                message: "下载失败",
...
//复制移动链接按钮点击判断
  buttonCopySelected = () => {
    const { currentPath, checkedData } = this.props.dataManager;
    //需要判断 当前用户是进入到哪个目录下方
    if (currentPath == "") {
      return false;
  ...
  render() {
    ...
    const { currentPath, testUsedSize, exampleUsedSize } = dataManager;
    ...
    const columns = this.state.status == false ?
      this.props.dataManager.currentPath.split("/")[0] == "示例数据" ? this.copyColumns : this.columns :
      this.props.dataManager.currentPath.split("/")[0] == "示例数据" ? this.columnsCopySearch : this.columnsSearch;

    return (
      <PageHeaderLayout
        title="数据管理"
        breadcrumbList={[{ title: "应用管理" }, { title: "数据管理" }]}
      >
          ...
          <div style={{height:"32px"}}>
            <div id="fixedTop" >
              <div id="top_btnMenu"    className={styles.top_btnMenu}>
                ...
                <FileTree {...this.props} dataManager={this.props.dataManager} buttonCopySelected={this.buttonCopySelected}
                          that={this} refresh={this.refresh} reFreshData={this.reFreshData}/>
              </div>
              <div className={styles.searchNav}  id="searchNav"  >
                ...
                <div className={styles.progressLine}>
                  <p><span style={{ marginRight: "59px" }}>示例数据{bytesToSize(exampleUsedSize)}</span>
                    <span>测试数据{bytesToSize(testUsedSize)}</span></p>
                </div>
              </div>
            </div>
          </div>
